...
* 用户选择微信支付后，商户需调用该接口在微信支付下单，生成用于调起支付的预支付交易会话标识(prepay_id)。
...
1. 用户在小程序上点击支付，请求到JSAPI下单接口，实际上最终会请求到我们所编写的PayController类中的jsapi方法，生成预支付id，同时将一些数据保存在prepay_order数据库中。
...
* yaml配置文件
  ...
  sky:
    wechat:
      ...
      secret: ************
      mchid: zhang666
      notifyUrl: http://111.119.211.126:9101/notify/paySuccess
  ```
