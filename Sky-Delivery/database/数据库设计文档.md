...
prepay_order表为预支付订单信息表，用于存储预支付订单信息。

| 字段名       | 数据类型     | 说明               | 备注     |
| ------------ | ------------ | ------------------ | -------- |
| id           | bigint       | 预支付id           | 主键     |
| total        | decimal(9,2) | 金额               | 非空     |
| currency     | varchar(16)  | 金额单位           | 非空     |
| out_trade_no | varchar(32)  | 商户系统内部订单号 | 非空     |
| openid       | varchar(128) | 微信用户唯一标识   | 非空     |
| description  | varchar(127) | 商品描述           | 非空     |
| notify_url   | varchar(255) | 支付回调通知URL    | 允许为空 |
| create_time  | datetime     | 创建时间           | 允许为空 |
| appid        | varchar(32)  | 公众号id           | 非空     |
| mchid        | varchar(32)  | 直连商户号         | 非空     |

### 13. transaction_record

transaction_record表是交易记录表，用来存储实际交易记录。

| 字段名       | 数据类型     | 说明               | 备注     |
| ------------ | ------------ | ------------------ | -------- |
| id           | bigint       | 交易id             | 自增主键 |
| total        | decimal(9,2) | 交易金额           | 非空     |
| currency     | varchar(16)  | 金额单位           | 非空     |
| out_trade_no | varchar(32)  | 商户系统内部订单号 | 非空     |
| openid       | varchar(128) | 微信用户唯一标识   | 非空     |
| description  | varchar(127) | 商品描述           | 非空     |
